...
> [!NOTE]
> The NinjaRoboticPlatform Ninja Code Assistant uses this robot profile only as browser-side context for Blockly guidance and future Tire/Humanoid/Spider-specific rules. The assistant's BYO AI provider API key is entered in the Code IDE browser session and is not stored on the robot, in `config.json`, in saved actions, or in BLE payloads.
